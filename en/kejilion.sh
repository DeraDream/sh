...
if [[ "$script_source" == /dev/fd/* || "$script_source" == /proc/self/fd/* || ! -f "$script_source" ]]; then
	...
	if ! curl -fsSL https://lion.yforward7.com/ -o "$install_tmp" || ! head -1 "$install_tmp" | grep -q '^#!/bin/bash'; then
		rm -f "$install_tmp"
		echo "Script download failed. Check the network and try again."
		exit 1
	fi
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}Advanced usage of k command${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}Return to main menu"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "Please enter your choice:" sub_choice
...
while true; do
	...
	echo "Change log"
	echo "------------------------"
	echo "All logs:${gh_proxy}raw.githubusercontent.com/DeraDream/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
...
uninstall_kejilion_script() {
	...
	echo "------------------------------------------------"
	read -e -p "This removes script files, backups, update jobs, and shortcuts. Continue? [y/N]: " uninstall_confirm
	if [[ ! "$uninstall_confirm" =~ ^[Yy]$ ]]; then
		echo "Uninstall cancelled."
		return
	fi
	read -e -p "Confirm again: permanently uninstall? [y/N]: " uninstall_confirm_again
	...
	echo "Script completely uninstalled."
	exit 0
}
...
echo -e "${gl_kjlan}01.  ${gl_bai}Uninstall script"
echo -e "${gl_kjlan}------------------------${gl_bai}"
