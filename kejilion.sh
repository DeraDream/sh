...
# 通过统一短链接安装时，根据参数切换到对应语言脚本。
case "${1:-}" in
	en|tw|jp|kr|ru|ir)
		language_code="$1"
		language_tmp=$(mktemp)
		language_url="https://raw.githubusercontent.com/DeraDream/sh/main/${language_code}/kejilion.sh"
		language_proxy_url="https://gh.kejilion.pro/raw.githubusercontent.com/DeraDream/sh/main/${language_code}/kejilion.sh"
		if ! curl -fsSL "$language_url" -o "$language_tmp"; then
			curl -fsSL "$language_proxy_url" -o "$language_tmp"
		fi
		if ! head -1 "$language_tmp" | grep -q '^#!/bin/bash'; then
			rm -f "$language_tmp"
			echo "语言脚本下载失败，请检查网络后重试"
			exit 1
		fi
		mv -f "$language_tmp" ~/kejilion.sh
		chmod +x ~/kejilion.sh
		exec bash ~/kejilion.sh
		;;
esac
...
bbrv3() {
		  ...
		  xanmod_add_repo() {
				...
				local fallback_key_url="${gh_proxy}raw.githubusercontent.com/DeraDream/sh/main/archive.key"
				local os_codename=""
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  6)
			  ...
			  clear
			  restore_defaults
			  curl -sS ${gh_proxy}raw.githubusercontent.com/DeraDream/sh/refs/heads/main/network-optimize.sh -o /tmp/network-optimize.sh && source /tmp/network-optimize.sh && restore_network_defaults
			  send_stats "还原默认设置"
			  ;;
		  ...
		  7)
			  cd ~
			  clear
			  curl -sS ${gh_proxy}raw.githubusercontent.com/DeraDream/sh/refs/heads/main/network-optimize.sh | bash
			  send_stats "内核自动调优"
			  ;;
		  ...
		  2)
			...
			19)
			  ...
			  if command -v apt >/dev/null 2>&1; then
				...
				echo "当前系统暂不支持自动安装 Ookla 官方 Speedtest CLI"
				continue
			  fi
			  clear
			  speedtest
			  send_stats "安装Speedtest官方测速工具"
			  ;;
			20)
			  clear
			  install iperf3
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  115|hermes)
	  	  bash <(curl -sL ${gh_proxy}raw.githubusercontent.com/DeraDream/sh/main/hermes_manager.sh)
		  ;;
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}raw.githubusercontent.com/DeraDream/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
